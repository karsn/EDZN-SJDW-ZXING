...
#include <algorithm>
#include <zxing/detector/FinderPatternFinder.h>
...
vector<Ref<FinderPattern> > FinderPatternFinder::find_ex(DecodeHints const& hints) {
  bool tryHarder = hints.getTryHarder();
  ...
  vector<Ref<FinderPattern> > result(possibleCenters_.size());
  for(int i=0; i<possibleCenters_.size(); i++)
  {
  	result[i] = possibleCenters_[i];
  }
  return result;
}
